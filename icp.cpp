...
void ICP(vector<Point3f>& pts1,  vector<Point3f>& pts2)
{
	int N = pts1.size();
	...
	while (err > 0.001)
	{
		...
		pts1_mid.clear();
		error.clear();
		calNearestPointPairs(R_12, T_12, pts1, pts2, pts1_mid, error);
...
void readTxt(vector<Eigen::Vector3f> &srcPoints)
{
	char filename[20];
